...
-- primary key 제약조건 에러 발생(null 값 이용)
INSERT
INTO user_primarykey
(user_no, user_id, user_pwd, user_name, gender, phone, email)
VALUES
...
-- [23000][1062] (conn=193) Duplicate entry '2' for key 'PRIMARY'
-- 프라이머리 키 제약 조건 에러코드
...
-- foreign key 제약쥬군 에러 발생(참조 컬럼에 없는 값 적용)
INSERT
INTO user_foreignkey1
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
VALUES
...
-- [23000][1452] (conn=193) Cannot add or update a child row:
...
FROM user_grade
WHERE grade_code = 10;
-- [23000][1451] (conn=193) Cannot delete or update a parent row:
-- a foreign key constraint fails
-- (`menudb`.`user_foreignkey1`, CONSTRAINT `user_foreignkey1_ibfk_1`
-- FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`))
...
/* 5. CHECK 제약 조건
    ...
    - SELECET의 WGERE절에 사용했던 조건식을 똑강이 사용
...
-- gender 컬럼의 CHECK 제약 조건 위배(남성 INSERT)
INSERT
INTO user_check
VALUES (null, '안중근', '남성', 27);
-- gender 컬럼의 CHECK 제약 조건 위배(나이가 19 미만)
INSERT
INTO user_check

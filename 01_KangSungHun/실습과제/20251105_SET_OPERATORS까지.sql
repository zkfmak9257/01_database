...
FROM EMPLOYEE E
         ...
         JOIN LOCATION L ON (L.LOCAL_CODE = D.LOCATION_ID)
WHERE J.JOB_NAME = '대리'
  AND L.LOCAL_NAME IN ('asia1', 'aisa2', 'asia3');


-- 9. 각 부서별 평균 급여와 직원 수를 조회하시오. (NULL 급여는 제외) <- 9번
...
-- 10. 직원 중 보너스를 받는 직원들의 연봉 총합이 1억 원을 <- 10번
...
WHERE E.BONUS IS NOT NULL
GROUP BY DEPT_TITLE
HAVING SUM(E.SALARY * (1 + E.BONUS) * 12) > 100000000;

-- 11. 국내 근무하는 직원들 중 평균 급여 이상을 받는 <- 11번
...
-- 13. 차장(J4) 이상 직급을 가진 직원과 사원(J7) 직급을 가진 <- 13번
-- 직원들의 급여 합계를 비교하여 결과를 출력하시오. (SET OPERATOR 사용) (2행)

SELECT '차장', SUM(e.SALARY)
FROM EMPLOYEE e

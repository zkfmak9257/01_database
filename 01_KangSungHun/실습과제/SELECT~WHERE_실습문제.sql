...
SELECT *
FROM tbl_menu
ORDER BY tbl_menu.category_code ASC, menu_price DESC;
...
-- 7번. tbl_menu 테이블에서 가격이 10,000원인 메뉴의 메뉴명과 가격을 조회하세요.
   SELECT menu_name, tbl_menu.menu_price
   FROM tbl_menu
...
-- 9번. tbl_menu 테이블에서 가격이 15,000원 이상인 메뉴의 메뉴코드, 메뉴명, 가격을 조회하세요.
    SELECT tbl_menu.menu_code,tbl_menu.menu_name,tbl_menu.menu_price
    FROM tbl_menu
...
-- 13번. tbl_menu 테이블에서 (카테고리 코드가 6번 또는 10번) 그리고 주문 가능한 메뉴를 조회하세요. (괄호 사용)
    SELECT *
    FROM tbl_menu
    WHERE (category_code = 6 OR category_code = 10)
    AND orderable_status = 'Y';
...
-- 16번. tbl_menu 테이블에서 가격이 10,000원 ~ 20,000원 범위가 아닌 메뉴를 가격순으로 조회하세요. (NOT BETWEEN 사용)
    SELECT *
    FROM tbl_menu
    WHERE tbl_menu.menu_price NOT BETWEEN 10000 AND 20000;
...
SELECT *
FROM tbl_menu
WHERE tbl_menu.menu_name LIKE '_%마%';

-- 21번. tbl_menu 테이블에서 메뉴명에 '김치'가 포함되지 않은 메뉴를 조회하세요. (NOT LIKE 사용)

SELECT *
FROM tbl_menu
WHERE tbl_menu.menu_name NOT LIKE '김치';

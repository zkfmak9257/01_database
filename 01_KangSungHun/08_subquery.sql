...
-- 3) SUBQUERY를 이용해서 딱 한 번의 SQL 실행으로 결과 찾기
SELECT menu_name, category_code
FROM tbl_menu
WHERE category_code = (
    SELECT category_code
...
/* SUBQUERY란 무어잇인가
   ...
   - 단일행, 단일열, 다중행, 다중열, 단중행 단중열, 스칼라, 상관, 인라인뷰
   - 서브쿼리를 위한 연산자 : EXISTS (존재하다.)
 */
-- 1) 단일행 서브쿼리
...
FROM tbl_menu
WHERE
    (tbl_menu.category_code, menu_price) = (
    SELECT tbl_menu.category_code, tbl_menu.menu_price
FROM tbl_menu
ORDER BY menu_price DESC
    LIMIT 9, 1);

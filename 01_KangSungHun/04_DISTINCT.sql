...
-- 결과 : NULL도 1개만 조회 -> == NULL의 중복도 제거한다.
SELECT DISTINCT ref_category_code
FROM tbl_category;

-- ref_category_code의 종류를 모두 조회
-- 단, NULL 제외
SELECT DISTINCT tbl_category.ref_category_code
FROM tbl_category
WHERE ref_category_code IS NOT NULL;

-- 여러 컬럼(다중 열)에서 DISTINCT 적용
-- 모든 컬럼 값이 같은 경우 중복 제거 (묶어서 비교하는 것)
SELECT DISTINCT category_code, orderable_status
FROM tbl_menu;

-- DISTINCT는 SELECT 절 제일 앞에만 작성 가능
SELECT  category_code, DISTINCT orderable_status
FROM tbl_menu;

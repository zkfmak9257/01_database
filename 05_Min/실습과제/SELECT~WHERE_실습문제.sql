...
SELECT
    orderable_status,menu_price
FROM
    tbl_menu
WHERE
    orderable_status = 'Y' && menu_price >= 5000;
-- 12번. tbl_menu 테이블에서 카테고리 코드가 4번이거나 가격이 9,000원인 메뉴를 조회하세요.
SELECT category_code,
       menu_price
FROM tbl_menu
WHERE category_code = '4' || menu_price = '9000';


-- 13번. tbl_menu 테이블에서 (카테고리 코드가 6번 또는 10번) 그리고 주문 가능한 메뉴를 조회하세요. (괄호 사용)
SELECT
    menu_name,category_code,orderable_status
FROM
    tbl_menu
WHERE
    (category_code = '6' || category_code = '10') && orderable_status = 'Y';
...
FROM
    tbl_menu
WHERE
    menu_price >= 8000 && menu_price <= 20000;
-- ========================================
-- WHERE 절 - BETWEEN
-- ========================================
...
SELECT
    menu_price,category_code,orderable_status
FROM
    tbl_menu
WHERE
    menu_price >= 7000 && category_code IN (4,6,10) && orderable_status = 'Y'
ORDER BY
    menu_price ASC;
...
FROM
    tbl_menu
WHERE
    (orderable_status = 'N' || menu_price < 10000)
    && (menu_name LIKE '%빵' || menu_name LIKE '%떡' || menu_name LIKE '%찜');
...
-- INNER JOIN시 : 하동훈, 이오리 결과 포함 X
-- LEFT [OUTTER] JOIN 시 : 하동운, 이오리 결과 포함 O
SELECT
    *
FROM
    employee e
...
/* 5. SELF JOIN
   - 같은 테이블의 JOIN
   - 같은 테이블의 행과 행 사이의 관계를 찾을 때 주로 사용한다
...
SELECT
    a.EMP_ID AS '사번',a.EMP_NAME AS '이름',NVL(b.EMP_NAME, '없음') AS '사수명'
FROM
    employee a
...
/* USING
   ...
   - 단점 : 다른 JOIN, SUBQUERY, 연산 수행 시
            어떤 테이블의 컬럼을 지시하려는 것 인지 알 수 없음 ( 애매모호함 )
*/

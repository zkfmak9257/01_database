...
/*
    ...
    - 종류 : 단일행(서브쿼리 결과 1), 다중행, 다중열, 단일행 다중열, 스칼라, 상관, 인라인뷰
    - 서브쿼리를 위한 연산자 : EXISTS
*/
...
/* 상관(상호연관 서브쿼리)
   - 메인 쿼리가 서브 쿼리의 결과에 영향을 주는 경우

   1) 메인 쿼리 1행 접근(커서)
   2) 서브쿼리에서 사용 할 메인쿼리의 컬럼값을 전달 (메인쿼리 -> 서브쿼리)
...
FROM (
    SELECT
    menu_name AS '메뉴명',category_name AS '카테고리명'
FROM
    tbl_menu a JOIN

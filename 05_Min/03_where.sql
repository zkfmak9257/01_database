...
-- %문자열 : 해당 문자열로 끝남 ( 앞에 무엇이 있던 %xxx) xxx를 포함하며 %자리는 무엇이던 상관 X
...
FROM
    tbl_menu
WHERE
    menu_name LIKE '_____'; -- 5글자 메뉴명만 조회 _ 개수로 정해짐


-- 마늘 앞에 꼭 1글자, 그 뒤는 아무거나
SELECT
    menu_name
FROM
    tbl_menu
...
-- IS NOT NULL : 해당 컬럼의 값이 NULL(빈칸) 이면 TRUE -> 결과 포함

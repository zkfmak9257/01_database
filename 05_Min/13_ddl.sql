...
CREATE TABLE IF NOT EXISTS tb1(
    pk INT PRIMARY KEY, -- PK는 행을 식별 할 수 있는 컬럼
    fk INT,
    col1 VARCHAR(255),
    CHECK ( col1 IN ('Y','N'))
) ENGINE = INNODB;
...
/* 테이블 생성 시 AUTO)_INCREMENT 적용
   - AUTO_INCREMENT란?
   - PRIMARY키에 해당하는 컬럼에 자동으로 번호를 발생(중복되지 않게)시켜 저장할 수 있다.

   - 사용법 : INSERT/UPDATE 수행 시
        AUTO_INCREMENT가 적용된 컬럼의 값으로 NULL을 대입하면 된다
*/
...
-- AUTO_INCREMENT는 PRIMARY KEY가 설정된 컬럼에서만 동작이 가능하다
-- -> PRIMARY KEY 삭제 시 AUTO_INCREMENT가 정상동작 X
-- 삭제 못하게 에러 발생
/*[42000][1075] (conn=56) Incorrect table definition;

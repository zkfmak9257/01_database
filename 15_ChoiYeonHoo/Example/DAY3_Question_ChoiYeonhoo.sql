...
-- 1. JOIN을 이용한 부서코드,부서명,지역명,국가명 추출 쿼리
SELECT
    d.DEPT_ID,
    d.DEPT_TITLE,
    l.LOCAL_NAME,
...
-- 2. 1에서 작성한 쿼리를 서브쿼리로 활용하여, 모든 직원의 사원번호, 직원명, 급여, 부서명, 국가명을 출력
WITH DEPARTMENT_SUBTABLE
    AS
    (
    SELECT
        d.DEPT_ID,
...
FROM
    employee e
    JOIN DEPARTMENT_SUBTABLE D ON (e.DEPT_CODE = D.DEPT_ID)
;

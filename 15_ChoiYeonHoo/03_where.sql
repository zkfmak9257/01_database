...
-- BETWEEN A AND B : A 이상 B이하 범위 지정
SELECT
    *
FROM
    tbl_menu
...
-- NOT BETWEEN A AND B : A 이상 B이하 범위가 아닌 것 지정
SELECT
    *
FROM
    tbl_menu
...
FROM
    tbl_menu
WHERE
    menu_name LIKE '_____'; -- 다섯글자 메뉴명만 조회
...
FROM
    tbl_menu
WHERE
    menu_name LIKE '_마늘%'; -- 마늘 앞에 한글자, 뒤는 상관 없음
...
SELECT *
FROM tbl_temp
WHERE temp_email LIKE '____%';  -- 원래 의도 : ___앞 세글자, _문자열을 포함한 데이터 출력
                                -- 실제 동작 : 모든 자료 다 나옴

-- ESCAPE OPTION을 이용해서 와일드카드 -> 문자열로 탈출
SELECT *
FROM tbl_temp
WHERE temp_email LIKE '___#_%' ESCAPE '#'; -- _, % 제외한 나머지 기호를 사용
...
FROM tbl_category
WHERE
    ref_category_code = NULL; -- 아무것도 조회가 안됨. NULL 비교 연산 불가

-- IS NULL : 해당 컬럼의 값이 NULL(빈칸) 이면 TRUE를 반환 -> 결과 포함
-- IS NOT NULL : 해당 컬럼의 값이 NULL(빈칸)이 아니면 TRUE를 반환 -> 결과 포함
SELECT
    category_name,
    ref_category_code
FROM tbl_category

...
/* 3. PRIMARY KEY
    ...
    - 테이블당 1개만 존재 가능
     (여러 컬럼을 묶어서 하나의 PK로 설정 가능 == 복합기)

    - UNIQUE(중복 X) + NOT NULL(빈칸 X)
     (효과가 같나는 뜻이지 U, NN 같이 설정해서 PK가 되진 않는다.)
...
/* 4. FOREIGN KEY (외래키)
    ...
    - 다른 테이블에서 제공되는 값 외에 NULL 사용 가능
      (단, FK, NOT NULL 같이 적용시 NULL 허용 X)
*/
...
/*
    ...
    a foreign key constraint fails (`menudb`.`user_foreignkey1`, CONSTRAINT `user_foreignkey1_ibfk_1`
    FOREIGN KEY (`grade_code`) REFERENCES `user_grade` (`grade_code`))
*/



-- user_goreignkey1.grade_code 컬럼에 NOT NULL 제약 조건 설정 x
-- -> NULL 삽입 가능
INSERT
  INTO user_foreignkey1
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
...
/*
    ...
    - 부모 테이블 : 참조를 당하는 테이블(값을 제공)
    - 자식 테이블 : 참조를 하는 테이블 (제공 받은 값을 이용)
*/


-- FK 삭제 룰 중 'SET NULL' 사용
...
CREATE TABLE IF NOT EXISTS user_foreignkey2 (
    ...
    FOREIGN KEY (grade_code)
		REFERENCES user_grade (grade_code)
        ON UPDATE SET NULL  -- 부모 값이 바뀌면 참조하는 자식의 값을 NULL변경
...
-- user_foreignkey1 테이블과의 관례로 인해 오류 발생
DROP TABLE IF EXISTS user_foreignkey1;
...
-- 20을 참조하던 컬럼 값이 null로 병경되었는지 확인
SELECT * FROM user_foreignkey2;
...
/* 5. CHECK 제약 조건
   ...
   - SELECT의 WHERE 절에 사용했던 조건식을 똑같이 사용

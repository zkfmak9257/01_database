...
SELECT MIN(emp_name), MAX(emp_name), MIN(HIRE_DATE) AS ' 가장 빠른 입사 일', MAX(HIRE_DATE) AS ' 가장 최근 입사 일'
FROM employee;
...
/*
 2. HAVING
 - GROUP BY로 만들어진 그루벵 대한 조건을 작성하는 절
...
SELECT category_code, SUM(menu_price) AS '합계'
FROM tbl_menu

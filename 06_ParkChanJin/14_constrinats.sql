...
/* 2. UNIQUE
   - UNIQEU 제약 조건이 설정된 컬럼 내에 동일한 값(중복 값)을
     허용하지 않는 제약 조건

   - NULL 중복 O (NULL은 값이 아니기 때문)
*/
...
/* 3. PRIMARY KEY
   ...
   - UNIQUE(중복 X) + NOT NULL(빈칸 X)
     (효과가 같다는 뜻이지 UNIQUE와 NOT NULL을 같이 설정해서 PRIMARY KEY가 되진 않는다)
*/
...
/* 4. FOREIGN KEY (외래키)
   ...
   - FK 제약 조건이 설정되면
     두 테이블 간의 관계(Realationship)가 형성됨
     (RDBMS(관계형 데이터베이스)에서 제일 중요한 개념)
...
-- user.foreignkeky1.grade_code 컬럼에 NOT NULL 제약 조건 설정 X
-- ->  NULL 삽입(사용) 가능
INSERT
  INTO user_foreignkey1
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
...
(3, 'user03', 'pass03', '이순신', '남',
 '010-777-7777', 'lee222@gmail.com', NULL);
SELECT * FROM user_foreignkey1;
...
CREATE TABLE IF NOT EXISTS user_foreignkey2 (
    ...
    FOREIGN KEY (grade_code)
		REFERENCES user_grade (grade_code)
        ON UPDATE SET NULL -- 부모 값이 바뀌면 참조하는 자식의 값을 NULL 로 변경
        ON DELETE SET NULL -- 부모 값이 삭제되면 참조하는 자식의 값을 NULL 로 변경
) ENGINE=INNODB;
...
-- user_foreignkey 테이블과의 관계로 인해 오류 발생
...
/* 5. CHECK 제약 조건
   - 해당 컬럼의 조건에 맞는 값만 삽입/수정할 수 있도록 확인하는 제약 조건
...
/* 6. DEFAULT (기본 값)
   - INSERT/UPDATE 수행 시 컬럼에 NULL을 삽입/수정하면
     대신 들어갈 기본 값을 설정
*/

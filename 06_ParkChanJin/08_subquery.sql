...
FROM
    tbl_menu
WHERE
    (category_code,menu_price) = (SELECT
...
-- 5) 스칼라(한개를 뜻함) 서브쿼리 (Scalar Subquery)
-- SELECT 절에서 사용되는 서브쿼리 (1행 1열만 반환)
-- SELECT 절에서 사용되는 단일행 서브쿼리
...
/*  EXISTS
    - 서브쿼리의 조회 결과가 있을 경우에만
      해당 메인쿼리 1행을 RESULT SET에 포함
*/


-- 실제로 메뉴 테이블에 사용된(등록된) 카테고리만 조회
-- 게시판에서 게시물 -> 댓글 구조를 구현할 때 많이 사용
SELECT
       category_name
  FROM tbl_category a

...
/*
    ...
    - 단일행 단일열, 다중행, 다중열, 다중행 다중열(행 여러개 열도 여러개), 스칼라, 상관, 인라인뷰
    - 서브쿼리를 위한 연산자 : EXISTS
...
SELECT menu_code
     ...
     , (SELECT category_name
        FROM tbl_category
        WHERE category_code = 1) AS category_name -- 단일행 , , , (단일열) == 스칼라
FROM tbl_menu m
ORDER BY menu_code
LIMIT 5;
...
SELECT menu_name
     , menu_price
     , (SELECT AVG(menu_price) FROM tbl_menu)  AS avg_price -- SELECT안에 서브쿼리 단일행 + SELECT 절 -> 스칼라
...
/*
    ...
    5) 메인쿼리가 전달 받은 값을 이용해 연산 수행
     - WHERE절 : 연산 수행 결과에 따라 메인쿼리 1행을 RESULT SET에 포함 할지 말지 결정
...
SELECT
    *
FROM tbl_menu
WHERE category_code = 4; -- group by랑 똑같은 결과
...
JOIN
    tbl_category b
            ON a.category_code = b.category_code
             ) AS menu_category  -- 테이블 별칭
WHERE `메뉴명` LIKE '%마늘%';



-- CTE 적용 : 메인, 서브쿼리 분리되어 편함 좀 더 속도가 빠름

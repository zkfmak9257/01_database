...
FROM
    tbl_menu
WHERE
    category_code IN (4,5,6,10); -- 카테고리 코드 안에 있는 거 포함
...
FROM
    tbl_menu
WHERE
    category_code NOT IN (4,5,6,10); -- 4,5,6,10 뺀 나머지
...
FROM
    tbl_category
WHERE
     ref_category_code = NULL; -- 아무것도 조회 X 비교 연산 불가
...
FROM
    tbl_category
WHERE
    ref_category_code IS NULL; -- 빈칸인 것들만 조회


-- IS NULL : 해당 컬럼의 값이 NULL(빈칸)이 아니면 TRUE -> 결과 포함
SELECT
    *
FROM
    tbl_category

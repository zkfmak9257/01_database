...
-- 3) 서브 쿼리를 이용해서 한 번의 SQL 실행으로 결과 찾기
SELECT
    menu_name,
    category_code
FROM
...
WHERE
    category_code IN (SELECT category_code
                      FROM tbl_category
                      WHERE ref_category_code = 1); -- ==category_code IN (4,5,6,7)
-- '식사'의 하위 카테고리
...
FROM
    tbl_menu
WHERE
    (category_code, menu_price) IN (SELECT category_code, MIN(menu_price)
                                    FROM tbl_menu
                                    GROUP BY
                                        category_code)
...
SELECT menu_code,
       ...
       (SELECT category_name
        FROM tbl_category
        WHERE category_code = m.category_code) AS category_name -- m.category_code
FROM tbl_menu m
ORDER BY menu_code
LIMIT 5;

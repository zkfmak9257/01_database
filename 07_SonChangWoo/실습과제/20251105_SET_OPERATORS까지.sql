...
SELECT
    E.EMP_NAME, J.JOB_NAME, E.SALARY, (E.SALARY * 12 * (1+NVL(E.BONUS, 0))) AS `연봉`
FROM employee E
JOIN
    job J ON(E.JOB_CODE = J.JOB_CODE)
JOIN
...
SELECT
    E.EMP_NAME, D.DEPT_TITLE, L.LOCAL_NAME, L.NATIONAL_CODE
FROM employee E
JOIN department D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN location L ON (D.LOCATION_ID = L.LOCAL_CODE)
...
SELECT
    D.DEPT_TITLE AS `부서명`, SUM(E.SALARY*(1+E.BONUS)*12) AS `연봉 총합`
FROM employee E
JOIN department D ON (E.DEPT_CODE = D.DEPT_ID)
WHERE E.BONUS IS NOT NULL
GROUP BY D.DEPT_TITLE
...
JOIN department D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN location L ON (D.LOCATION_ID = L.LOCAL_CODE)
WHERE L.NATIONAL_CODE = 'KO' AND E.SALARY >= (SELECT AVG(SALARY) FROM employee);

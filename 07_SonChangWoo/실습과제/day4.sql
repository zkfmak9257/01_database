...
CREATE TABLE IF NOT EXISTS TEAM_INFO(
    ...
    team_name VARCHAR(100) NOT NULL COMMENT '소속명', -- * : NOT NULL
...
SELECT * FROM TEAM_INFO; -- 삽입 제대로 됬는지 확인
...
SELECT * FROM member_info; -- 삽입 제대로 됬는지 확인

/*
단합을 위한 사내 체육 대회를 위하여 팀을 꾸리는 중입니다.
기술 지원부의 대리, 인사 관리부의 사원, 영업부(팀명에 ‘영업’이 포함되면 영업부로 봄)의 부장을
한팀으로 묶으려고 합니다. 이때, 이팀의 팀원 수를 출력하세요.
단, UNION과 SUBQUERY를 활용하여 출력하세요.
*/
SELECT COUNT(*) AS "팀원_수"
FROM (
SELECT E.EMP_ID, DEPT_TITLE, JOB_NAME
FROM employee E JOIN department D ON E.DEPT_CODE = DEPT_ID
...
FROM employee E JOIN department D ON E.DEPT_CODE = DEPT_ID
JOIN job J ON E.JOB_CODE = J.JOB_CODE
WHERE D.DEPT_TITLE LIKE '%영업%' AND J.JOB_NAME = '부장') AS TEAM_NUM;

...
-- (참고) 실수 -> 정수 변환 시 소수점 반올림 자동 수정
SELECT
    CAST(AVG(menu_price) AS SIGNED INTEGER) AS '메뉴 평균 가격'
FROM
    tbl_menu;

-- CONVERT()를 이용한 형변환 (ANSI 표준)
SELECT
    CONVERT(AVG(menu_price), SIGNED INTEGER) AS '메뉴 평균 가격'
FROM
    tbl_menu;

SELECT
    SUM(menu_price), CAST(SUM(menu_price) AS CHAR)
FROM
    tbl_menu;

...
CREATE TABLE IF NOT EXISTS user_unique (
    ...
    user_name VARCHAR(255) NOT NULL,
    gender VARCHAR(3),
    phone VARCHAR(255) NOT NULL /*UNIQUE*/, -- 컬럼 레벨 제약 조건 여러개 가능
    email VARCHAR(255),
...
/* 14-3. PRIMARY KEY
   ...
   - UNIQUE(중복 X) + NOT NULL(빈칸 X)
     (효과가 같다는 뜻이지 U, NN 같이 설정해서 PK가 되지 않는다)
*/
...
/* 14-4. FOREIGN KEY (외래키)
   ...
   - 다른 테이블에서 제공되는 값 외에 NULL 사용 가능
     (단, FK, NOT NULL이 같이 적용 시 NULL 허용 X)
*/
...
SELECT * FROM user_foreignkey1; -- 삽입 안된거 확인

-- user_foreignkey1.grade.code 컬럼에 NOT NULL 제약 조건 설정 X
-- -> NULL 삽입 가능
INSERT
  INTO user_foreignkey1
(user_no, user_id, user_pwd, user_name, gender, phone, email, grade_code)
...
SELECT * FROM user_foreignkey1; -- NULL 삽입된거 확인

-- 삭제 룰을 별도 설정하지 않았을 경우(RESTRICT)
DELETE
FROM user_grade
WHERE grade_code = 10;
/* 애러 메시지
[23000][1451] (conn=26) Cannot delete or update a parent row:
...
CREATE TABLE IF NOT EXISTS user_foreignkey2 (
    ...
    FOREIGN KEY (grade_code)
		REFERENCES user_grade (grade_code)
        ON UPDATE SET NULL  -- 부모 값이 바뀌면 참조하는 자식의 값을 NULL 변경
        ON DELETE SET NULL  -- 부모 값이 삭제되면 참조하는 자식의 값을 NULL 변경
) ENGINE=INNODB;
...
-- (user_foreignkey1 테이블 DROP하고 진행할 것(user_foreignkey에는
-- foreign key 제약조건에 수정 및 삭제룰 적용이 되지 않았기 때문)

-- SET NULL 삭제 옵션 테스트
-- (중요) user_foreignkey1 테이블 삭제 후 테스트 수행
--      왜? 삭제 안하면 user_grade 수정/삭제 시
...
-- 20을 참조하던 컬럼 값이 null로 변경되어 있는지 확인
SELECT * from user_foreignkey2;
...
/*
비정상 INSERT
1. gender 컬럼의 CHECK 제약 조건 에러 발생(성별이 두 글자)
*/
-- [애러 메시지]
-- CONSTRAINT `user_check.gender` failed for `menudb`.`user_check`
INSERT
  INTO user_check
...
-- [애러 메시지]
-- CONSTRAINT `user_check.age` failed for `menudb`.`user_check`
INSERT
  INTO user_check
VALUES (null, '유관순', '여', 17);

/* 14-6. DEFAULT(기본 값)
   - INSERT/UPDATE 수행시 컬럼에 NULL을 삽입/수정하면 대신 들어갈 기본 값을 설정
*/

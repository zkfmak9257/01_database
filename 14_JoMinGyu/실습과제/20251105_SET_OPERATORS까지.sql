...
where j.JOB_NAME = '대리' and l.LOCAL_NAME like 'ASIA_';
...
select d.DEPT_TITLE, e.SALARY*(1+e.BONUS)*12 annual_salary
from employee e
    join department d on e.DEPT_CODE = d.DEPT_ID
where e.BONUS is not null and e.SALARY*(1+e.BONUS)*12 > 100000000;
...
select d.DEPT_TITLE, count(e.EMP_NAME)
from employee e
...
select '차장 이상' as "구분", sum(SALARY) "급여_합계"
from employee e
where e.JOB_CODE <= 'J4'
union
select '사원' as "구분", sum(SALARY) "급여_합계"
from employee e
where e.JOB_CODE = 'J7';

...
-- '민트미역국'과 같은 같은 카테고리의 메뉴를 모두 조회하시오

-- 1) '민트미역국'의 마테고리 코드 찾기
...
SUBQUERY
    ...
    - 단일행, 다중행, 다중열, 단일행 다중열, 스칼라, 상관, 인라인뷰...
    - 서브쿼리를 위한 연산자 - EXISTS
*/
...
/*
EXISTS : 서브쿼리의 조회 결과가 있을 경우에만 해당 메인 쿼리 1행을 RESULT SET에 포함 시키는 연산자
*/
-- 실제로 메뉴 테이블에 등록된 카테고리만 조회
SELECT category_name
...
CTE(Common Table Expression)
    - 인라인 뷰 로 사용되는 서브쿼리를 미리 정의해서 사용하는 방법
    - 인라인 뷰 : FROM절에 사용되는 서브쿼리. 서브쿼리의 결과를 테이블 처럼 사용
*/
...
SELECT
    메뉴명, 카테고리명
FROM (
    SELECT menu_name AS '메뉴명', category_name AS '카테고리명'
    FROM tbl_menu a
    JOIN tbl_category b

...
SELECT *
FROM tbl_menu
WHERE (category_code = 6 OR category_code = 10) AND orderable_status = 'Y';

-- 14번. tbl_menu 테이블에서 가격이 8,000원 이상 20,000원 이하인 메뉴를 조회하세요. (AND 사용)
SELECT *
FROM tbl_menu
WHERE menu_price >= 8000 AND menu_price <= 20000;
...
SELECT *
FROM tbl_menu
WHERE orderable_status = 'N' OR (menu_price < 10000 AND (menu_name LIKE '%빵' OR menu_name LIKE '%떡'OR menu_name LIKE '%찜'));
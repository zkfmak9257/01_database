...
/* JOIN은 두 테이블에서 지정된 컬럼(FOREIGN KEY) 값이 같은 행들의 결합으로 만들어진 큰 테이블 */
...
/* INNER JOIN (ANSI) */
SELECT e.EMP_ID, EMP_NAME, d.DEPT_TITLE
FROM EMPLOYEE e
...
USING
    ...
    - 장점 : JOIN 결과 컬럼 개수 감소
    - 단점 : 다른 JOIN, SUBQUERY, 연산 수행 시 어떤 테이블의 컬럼을 지시하려는 것인지 알 수 없음
*/
SELECT *
FROM tbl_menu a
JOIN tbl_category b

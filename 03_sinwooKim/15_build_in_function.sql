...
-- 1. 문자열 함수
-- ASCII() : 문자의 아스키 코드 번호 반환
-- CHAR() : 입력한 아스키 코드에 맞는 문자를 반환
...
TRIM: TRIM은 기본적으로 앞뒤 공백을 제거하지만 방향(LEADING(앞), BOTH(양쪽),
TRAILING(뒤))이 있으면 해당 방향에 지정한 문자열을 제거할 수 있다.
*/

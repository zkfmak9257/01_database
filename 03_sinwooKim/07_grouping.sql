...
/* 그룹 함수
    - 전체 테이블 또는 GROUP이 지어진 결과 집함에 사용 가능한 함수
...
SELECT MIN(EMP_NAME), MAX(EMP_NAME), MIN(HIRE_DATE) AS '가장빠른입사일', MAX(HIRE_DATE) AS '가장최근입사일'
FROM EMPLOYEE;

-- swcamp 계정
/*
GROUP 내 GROUP 만들기 (2개 이상의 그룹 생성)
    - 큰 그룹 내 소규모 그룹 구성
*/
SELECT category_code, menu_price, -- menu_name
        COUNT(*)
            -- GROUP BY 가장 끝에 작성된 그룹을 기준으로 COUNT()함수 수행
FROM tbl_menu
...
/*
HAVING
    - GROUP BY로 만들어진 GROUP에 대한 조건을 작성하는 절

...
SELECT now() AS '조회시각';
/* SELECT
    menu_code AS '2025-11-04',
    menu_price AS '10:40',
    menu_code * menu_price AS '조회시각'
FROM
    tbl_menu
ORDER BY
    `조회시각` DESC;
...
-- 12번. tbl_menu 테이블에서 카테고리 코드가 4번이거나 가격이 9,000원인 메뉴를 조회하세요.
   SELECT
    ...
    FROM
        tbl_menu
    WHERE
         menu_price = 9000 OR category_code = 4;
...
-- 14번. tbl_menu 테이블에서 가격이 8,000원 이상 20,000원 이하인 메뉴를 조회하세요. (AND 사용)
    ...
    FROM
          tbl_menu
    WHERE
          menu_price >= 8000 AND menu_price <= 20000;
...
FROM
    tbl_menu
 WHERE
    menu_code IN (4,6,10) AND menu_price >= 7000
ORDER BY
    menu_code;
...
SELECT *
FROM tbl_menu
WHERE (orderable_status = 'N' OR price < 10000)
  AND (
        menu_name LIKE '%빵'
        OR menu_name LIKE '%떡'
        OR menu_name LIKE '%찜'

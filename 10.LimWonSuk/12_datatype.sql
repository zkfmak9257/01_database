...
-- 메뉴 가격 문자열로 바꿔서 조회
-- (참고) 문자열로 변환 시 숫자의 크기가 문자열의 크기보다 크면 문자열의 크기가 맞춰 잘림
SELECT
    COUNT(menu_price AS CHAR(4))
FROM
    tbl_menu;

SELECT
    CONCAT(menu_price, '원') AS 가격표시
FROM
    tbl_menu;
/*
 2. 암시적(묵시적) 형변환
    - 연산 수행시 자동으로 데이터 타입을 맞춰줌
 문자끼리 연산이안되는데 암묵적으로 숫자는 문자여도 해줌
 */

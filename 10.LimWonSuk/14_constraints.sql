...
CREATE TABLE IF NOT EXISTS user_notnull (
    user_no
        INT NOT NULL, -- 컬럼 레벨 제약 조건  ("설정할때 옆에 두겠다")
    user_id
...
(
    ...
    gender
        VARCHAR(3),
    phone
        VARCHAR(255) NOT NULL /*UNIQUE*/, -- 컬럼 레벨 제약 조건
    email
        VARCHAR(255),
...
/*
 ...
 (효과가 같다는 뜻이지 UNIQUE, NOT NULL 같이 설정해서 PK가 되진 않는다
 */
...
-- USER_FOREIGNKEY1.HRADE_CODE 컬럼에 NOT NULL 제약 조건 설정 X
-- -> NULL 삽입 가능


-- 삭제물이 설정되지 않은 경우 (RESTRICT)
SELECT *
FROM user_grade;
...
CREATE TABLE IF NOT EXISTS user_foreignkey2 (
    ...
    FOREIGN KEY (grade_code)
        ...
        ON DELETE SET NULL -- 부모 값이 바뀌면 참조하는 자식의 값을 NULL로 변경
) ENGINE=INNODB;
...
-- SET NULL 삭제 옵션 테스트
-- (중요) USER_FOREIGNKEY1 테이블 삭제후 테스트 수행
...
-- 비정상 INSERT (gender컬럼의 CHECK 제약 조건 위배('남성' 이라고 INSERT)
-- [23000][4025] (conn=120) CONSTRAINT `user_check.gender` failed for `menudb`.`user_check`
INSERT
  INTO user_check
VALUES (null, '안중근', '남성', 27);
-- 비정상 INSERT(age컬럼의 CHECK 제약조건 위배(나이가 19 미만)
-- [23000][4025] (conn=120) CONSTRAINT `user_check.age` failed for `menudb`.`user_check`
INSERT
  INTO user_check
...
CREATE TABLE IF NOT EXISTS tbl_country (
    ...
    country_name
        VARCHAR(255) DEFAULT '한국',
    population
        VARCHAR(255) DEFAULT '0명',
    add_day

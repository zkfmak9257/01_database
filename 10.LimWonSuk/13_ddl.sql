/*
 ...
 CREATE TABLE [IF NOT EXISTS] 테이블명 (
  ...
  컬럼명2 데이터타입 [제약조건],
 */
-- DROP TABLE [테이블이름] - 실수하면 이걸로 떨궈라
 CREATE TABLE tb1(
     pk INT PRIMARY KEY, -- pk는 행을 식별할 수 있는 컬럼
     fk INT,
     coll VARCHAR(255),
     CHECK (coll IN ('Y','N'))
 );

-- IF NOT EXISTS -> 테이블이 만약 존재하지 않으면 수행, 이미테이블 존재시 만들기x
...
/*
 ...
 - AUTO_INCREMENT란?
 PRIMARY키에 해당하는 컬럼에 자동으로 번호를 발색(중복되지 않게)시켜 저장

 -사용법 : INSERT/UPDATE 수행 시
 AUTO_INCREMENT가 적용된 컬럼의 값으로 NULL 대임
 */
...
-- pk컬럼 값을 NULL로 지정해서 INSERT (AUTO_INCREMENT 확인)
INSERT INTO tb2 VALUES (NULL, 10, 'Y') ; -- 어디에 들어갈지 미리보기
...
INSERT INTO tb2 VALUES (9, 40, 'Y') ;
INSERT INTO tb2 VALUES (11, 40, 'Y') ;
INSERT INTO tb2 VALUES (13, 40, 'Y') ;
...
-- 2-3. 열 수정
ALTER TABLE tb2
CHANGE COLUMN fk change_fk; -- 이러면 안됨
...
/*
 AUTO_INCREMENT는 PRIMARY KEY가 설정된 컬럼에서만 동작이가능
...
-- tb3 테이블 삭제
DROP TABLE tb3;
DROP TABLE IF EXISTS tb3;  -- 존재하면 지워라
...
SELECT * FROM tb6;
ROLLBACK;
SELECT * FROM tb6; -- 롤백확인 -> 복구 X

/*
 (참고)
 -DML(INSERT,UPDATE, DELETE, REPOLACE) 수행 후
 -DDL(CREATE, ALTER, DROP, TRUMCATE)을 수행하면
 자동으로 COMMIT이 진행된다! -> 이후 ROLL BACK불가
 */
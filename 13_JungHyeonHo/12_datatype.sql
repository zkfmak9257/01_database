...
SELECT CAST(AVG(menu_price) AS SIGNED INTEGER) '메뉴평균가격'
FROM tbl_menu;

-- CONVERT()를 이용한 형변환 (MySQL, MariaDB)
SELECT CONVERT(AVG(menu_price), SIGNED INTEGER) '메뉴평균가격'
FROM tbl_menu;

-- 결과값에서 문자형은 왼쪽에 붙고, 숫자형은 오른쪽에 붙는다
SELECT SUM(menu_price),
       CAST(SUM(menu_price) AS CHAR) 메뉴가격합계
FROM tbl_menu;

-- SIGNED INTEGER, UNSIGNED INTEGER 차이
-- SIGNED : 부호(+-)를 포함한 범위의 INTEGER (-21.47억 ~ +21.47억)
-- UNSIGNED : 부호(+-)를 포함하지 않는 범위의 INTEGER (0 ~ 42.9억)
--  -> 음수(-)를 UNSIGNED로 변환 시 무조건 0이 된다
...
--       문자열 크기에 맞춰서 숫자가 잘림
SELECT CAST(menu_price AS CHAR(4)) 메뉴가격
FROM tbl_menu
ORDER BY tbl_menu.menu_price ASC;

SELECT CONCAT(menu_price, '원') 메뉴가격
FROM tbl_menu;
SELECT CONCAT(CAST(menu_price AS CHAR(5)), '원') 메뉴가격
FROM tbl_menu;
...
FROM tbl_menu; -- menu_price가 문자로 변환됨
